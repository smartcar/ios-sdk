...
class SmartCarOAuthSDK {
    ...
    */
    ...
    /**
        Initializes the Authorization request and configures and return an SFSafariViewController with the correct authorization URL
     
        - Parameter oem: OEM object to identify the oem name within the authorization request URL
     
        - Returns: SFSafariViewController object configured with the authorization URL corresponding to the request paramters and the provide OEM
    */
    ...
    */
    func generateLink(for oem: OEM) -> String {
        ...
        let scopeString = self.request.scope.joined(separator: " ").addingPercentEncoding( withAllowedCharacters: .urlQueryAllowed)!
    ...
    /**
        Authorization callback function. Verifies the state parameter of the URL matches the request state paramter and extract the authorization code
